...
# 세그먼트 트리의 데이터 개수는 배열(arr)의 개수가 N개일 때, N보다 큰 가장 가까운 N의 제곱수를 구한 뒤에
# 그것의 2배까지 미리 배열의 크기를 미리 만들어놓아야 한다.
# 위 예제는 N=10으로 가장 가까운 제곱수는 4^2 = 16으로 16 * 2 = 32개의 크기가 필요하다.
# 그래서 실제로는 데이터의 개수 N에 4를 곱한 크기만큼 미리 세그먼트 트리의 공간을 할당한다. --> 저는 수학적으로 정확히 증명못했음... (다른 경우의 수를 해보니 맞음)
tree = [0] * (len(arr) * 4)
...
# 이때, 해당 원소를 포함하고 있는 모든 구간 합 노드들을 갱신해주면 된다. (즉, 전체가 아닌 부분적인 노드들만 바꿔주면 된다!)
# start : 시작 인덱스, end : 마지막 인덱스
